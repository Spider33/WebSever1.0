...
WebServer::WebServer(
    ...
    assert(srcDir_);
    strncat(srcDir_, "/resources", 11);
    HttpConn::userCount = 0;
    HttpConn::srcDir = srcDir_;
    SqlConnPool::Instance()->Init("localhost", sqlPort, sqlUser, sqlPwd, dbName, connPoolNum);  //单例模式
    
    InitEventMode_(trigMode);   //初始化事件触发方式
    if(!InitSocket_()) { isClose_ = true;}  //初始化socket连接
...
WebServer::~WebServer() {
    ...
    isClose_ = true;
    free(srcDir_);  //释放http连接指针
    SqlConnPool::Instance()->ClosePool();   //关闭sql池
}
...
bool WebServer::InitSocket_() {        
    ...
    SetFdNonblock(listenFd_);   //设置非阻塞IO
